...
namespace ISO22133 {
TestObject::TestObject(const std::string& listenIP, const std::string& name)
	: name(name),
	  trajDecoder(),
...
void TestObject::receiveTCP() {
	...
	while (this->on) {
		...
		if (!this->socketsReceivedFromController) {
			ss.str(std::string());
			ss << "Awaiting TCP connection from ATOS...";
			...
			<< " established.";
			// std::cout << "TCP connection to ATOS running at " << ctrlChannel.getEndPoint().address().to_string()
